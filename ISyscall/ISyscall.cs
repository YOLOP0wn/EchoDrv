...
namespace ISyscall
{
    public class Isyscall
    {
        ...
        delegate Data.NTSTATUS NtReadVirtualMemory(IntPtr processHandle, IntPtr baseAddress, byte[] buffer, UInt32 bytesToRead, ref uint bytesRead);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate UInt32 NtWriteVirtualMemory(IntPtr ProcessHandle, IntPtr BaseAddress, IntPtr Buffer, UInt32 BufferLength, out UInt32 BytesWritten);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate Data.NTSTATUS NtProtectVirtualMemory(IntPtr processHandle, ref IntPtr baseAddress, ref IntPtr regionSize, uint newProtect, out uint oldProtect);
        ...
        private SortedDictionary<long, string> buildNtTable()
        {
            ...
            var dosHeader = Marshal.PtrToStructure<ISyscall.PE.IMAGE_DOS_HEADER>(modptr);
